function [image, header] = mhdread(filepath, header_only)
if(~exist('header_only','var')), header_only = false; end
...
for fieldname = fieldnames(header)'
    ...
    switch(tag)
        ...
        case 'ElementSpacing'
            header.(tag) = str2num(header.(tag)); %#ok<ST2NM>
        case {'StudyDate', 'SOD', 'SDD', 'PositionerPrimaryAngle', 'PositionerSecondaryAngle'}
            header.(tag) = str2num(header.(tag)); %#ok<ST2NM>
        case 'PhotometricInterpretation'
            header.(tag) = header.(tag); %#ok<ST2NM>
        case 'DimSize'
            header.(tag) = str2num(header.(tag)); %#ok<ST2NM>
        case 'HeaderSize'
            header.(tag) = str2num(header.(tag)); %#ok<ST2NM>
...
fclose(fid);
if(header_only), image = []; return; end
